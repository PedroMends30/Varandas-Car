...
<body>
    ...
    <main class="pagina">
		...
		<section id="innerpagina">
			<form action="rpsEdit.jsp?id=<%=pec.getId_peca()%>" method="post">
				<div class="contentgrid">
					<h3>Veículo</h3>
					<div class="gridveiculo">
						<div class="contentinput">
							<label for="montadora">Editar Veículo</label>
							<input type="text" name="montadora" onblur="today()" required="" id="montadora" value="<%=vei.getMontadora_veiculo()%>">
						</div>
						<div class="contentinput">
							<label for="modelo">Editar Modelo</label>
							<input type="text" name="modelo" required="" id="modelo" value="<%=vei.getModelo_veiculo()%>">
						</div>
						<div class="contentinput">
							<label for="ano">Editar Ano de Lançamento</label>
							<input type="number" name="ano" max="2024" min="1960" required="" id="ano" placeholder="Digite o ano do veiculo" value="<%=vei.getAno_veiculo()%>">
						</div>
					</div>
				</div>
				<div class="contentgrid">
					...
					<div class="gridveiculo">
						<div class="contentinput">
							...
							<input type="text" name="nome_peca" required="" id="peca" value="<%=pec.getNome_peca()%>">
						</div>
						<div class="contentinput">
							<label for="dimencoes">Editar Volume</label>
							<input type="text" name="volume" required="" id="dimencoes" value="<%=pec.getVolume_peca()%>">
						</div>
						<div class="contentinput">
							<label for="cor">Editar Cor</label>
							<input type="color" name="cor" required="" id="cor" value="<%=pec.getCor_peca()%>">
						</div>
					</div>
					<div class="contentinput textarea">
						<label for="descricao">Editar Descrição</label>
						<textarea id="descricao" name="descricao" required="" cols="30" rows="10" value="<%=pec.getDesc_peca()%>"></textarea>
					</div>
